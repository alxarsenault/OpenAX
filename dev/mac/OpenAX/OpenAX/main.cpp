...
public:
	...
	void AddProperty(const ax::widget::ParamType& type, const std::string& name, void* handle)
	{
		_properties.insert(std::pair<std::string, ax::widget::ParamType>(name, type));
		_property_handles.insert(std::pair<std::string, void*>(name, handle));
	}

	void SetAttributeValue(const std::pair<std::string, std::string>& att)
	{
		// Search attribute name.
		auto p = _properties.find(att.first);

		if (p == _properties.end()) {
			return;
		}

		const ax::widget::ParamType& type = p->second;

		switch (type) {
		case ax::widget::ParamType::INTEGER:
			break;

		case ax::widget::ParamType::REAL:
			break;

		case ax::widget::ParamType::TEXT:
			break;

		case ax::widget::ParamType::FILEPATH:
			break;

		case ax::widget::ParamType::COLOR:
			*static_cast<ax::Color*>(_property_handles[att.first]) = ax::Xml::StringToColor(att.second);
			break;

		case ax::widget::ParamType::ALIGNEMENT:
			break;

		case ax::widget::ParamType::RANGE:
			break;

		case ax::widget::ParamType::POINT:
			*static_cast<ax::Point*>(_property_handles[att.first]) = ax::Xml::StringToSize(att.second);
			break;

		case ax::widget::ParamType::SIZE:
			*static_cast<ax::Size*>(_property_handles[att.first]) = ax::Xml::StringToSize(att.second);
			break;

		case ax::widget::ParamType::BOOLEAN:
			break;

		case ax::widget::ParamType::CUSTOM:
			break;
		}
	}
	...
	{
		return *static_cast<T*>(_property_handles[name]);
	}
	
	std::string GetAttributeValue(const std::string& name)
	{
		// Search attribute name.
		auto p = _properties.find(name);
		...
		switch (type) {
			...
			case ax::widget::ParamType::COLOR:
				return static_cast<ax::Color*>(_property_handles[name])->ToString();
				break;
			...
			case ax::widget::ParamType::POINT:
				return std::to_string(*static_cast<ax::Point*>(_property_handles[name]));
				break;
				
			case ax::widget::ParamType::SIZE:
				return std::to_string(*static_cast<ax::Size*>(_property_handles[name]));
				break;
...
private:
	std::map<std::string, ax::widget::ParamType> _properties;
...
int main()
{
	...
	p_tree.AddProperty(ax::widget::ParamType::POINT, "p1", &p1);
	
	p_tree.SetAttributeValue(std::pair<std::string, std::string>("p1", "2, 3"));
	ax::Print(std::to_string(p1));
	
	ax::Point& p1_test = p_tree.GetAttribute<ax::Point>("p1");
	p1_test.x = 1234;
	ax::Print(std::to_string(p1));
	
//	ax::Print(std::to_string(p1_test));
	//	ax::Point p1(12, 134);
	//
	...
	//	props["p1"] = &p1;
	//
	//	(*static_cast<ax::Point*>(props["p1"])).x = 84;
	//
	//	ax::Print(std::to_string(p1));
	//
	//	ax::Print(std::to_string(*static_cast<ax::Point*>(props["p1"])));
...
////
////		auto btn
////			= ax::shared<ax::Button>(ax::Rect(20, 20, 60, 25), ax::Button::Events(), btn_info, "",
///"Banana");
////		win->node.Add(btn);
////
