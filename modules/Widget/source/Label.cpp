...
Label::Info::Info(const ax::StringPairVector& attributes)
{
...
std::string Label::Info::GetAttributeValue(const std::string& name)
{
	if (name == "normal") {
		...
		if (alignement == ax::Utils::axALIGN_LEFT) {
			return "left";
		}
		else if (alignement == ax::Utils::axALIGN_CENTER) {
			return "center";
		}
		else if (alignement == ax::Utils::axALIGN_RIGHT) {
			return "right";
		}
	}
...
void Label::Info::SetAttribute(const ax::StringPair& attribute)
{
	if (attribute.first == "normal") {
		...
		if (attribute.second == "left") {
			alignement = ax::Utils::axALIGN_LEFT;
		}
		else if (attribute.second == "center") {
			alignement = ax::Utils::axALIGN_CENTER;
		}
		else if (attribute.second == "right") {
			alignement = ax::Utils::axALIGN_RIGHT;
		}
	}
}
...
ax::StringPairVector Label::Component::GetBuilderAttributes()
{
	ax::Window* win = GetWindow();
	std::shared_ptr<ax::Window::Backbone> bbone = win->backbone;
 ...
 */
Label::Label(
	const ax::Rect& rect, const Label::Info& info, const std::string& label)
	: _label(label)
	, _font(nullptr)
{
...
void Label::OnPaint(ax::GC gc)
{
	...
	if (info.alignement == ax::Utils::axALIGN_CENTER) {
		gc.DrawStringAlignedCenter(*_font, _label, rect);
	}
	else if (info.alignement == ax::Utils::axALIGN_LEFT) {
