 ...
 * TextBox::Flags.
 */
const ax::Flag TextBox::Flags::FLASHING_CURSOR = ax::FlagValue::FLAG_1;
const ax::Flag TextBox::Flags::CONTOUR_HIGHLIGHT = ax::FlagValue::FLAG_2;
const ax::Flag TextBox::Flags::CONTOUR_NO_FADE = ax::FlagValue::FLAG_3;
const ax::Flag TextBox::Flags::HIDDEN_TEXT = ax::FlagValue::FLAG_4;
...
TextBox::Info::Info(const ax::StringPairVector& attributes)
{
...
void TextBox::OnKeyDown(const char& key)
{
	...
	// Insert char in _label.
		...
		if (_label.size() != _maxNumChar) {
			if (_lastCharXPosition < win->dimension.GetRect().size.x - 10) {
