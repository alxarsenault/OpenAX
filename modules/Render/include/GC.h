...
public:
	...
	void DrawQuarterCircle(const ax::FloatPoint& pos, const int& radius,
	...
	void DrawRoundedRectangleContourSmooth(
	...
	void DrawPartOfImage(ax::Image* img, const ax::Point& posInImage,
		const ax::Size& sizeInImage, const ax::Point& pos, float alpha = 1.0);

	void DrawPartOfImageResize(ax::Image* img, const ax::Point& posInImage,
		const ax::Size& sizeInImage, const ax::Rect& rect, float alpha = 1.0);
	...
	void DrawStringAlignedLeft(
		ax::Font& font, const std::string& text, const ax::Rect& rect);

	void DrawStringAlignedCenter(
		ax::Font& font, const std::string& text, const ax::Rect& rect);

	void DrawStringAlignedRight(
		ax::Font& font, const std::string& text, const ax::Rect& rect);

	void DrawStringAlignedHorizontalCenter(
		ax::Font& font, const std::string& text, const ax::Rect& rect);

	void DrawRectangleColorFade(const ax::Rect& rect, const ax::Color& c1,
		const float& alpha1, const ax::Color& c2, const float& alpha2);

	void DrawRectangleColorFade(
		const ax::Rect& rect, const ax::Color& c1, const ax::Color& c2);

	void DrawRectangleVerticalColorFade(
		const ax::Rect& rect, const ax::Color& c1, const ax::Color& c2);

	// Order : bl, tl, tr, br.
	void DrawRectangleColorFade(
		const ax::Rect& rect, const std::vector<ax::Color>& colors);

	void DrawPolygone(const std::vector<ax::Point>& points);

	void DrawArrow(const ax::Point& p1, const ax::Point& p2,
		const double& arrow_length = 12, const double& base_length = 8,
		const double& width = 1);

	void DrawTriangleColorFade(const ax::Point& p1, const ax::Point& p2,
		const ax::Point& p3, const ax::Color& c1, const ax::Color& c2,
		const ax::Color& c3);

	void DrawLine(
		const ax::Point& pt1, const ax::Point& pt2, float width = 1.0);

	void DrawLines(const std::vector<ax::Point>& points,
		const std::vector<ax::Color>& colors, float width = 1.0);

	void DrawLineStripple(const ax::Point& pt1, const ax::Point& pt2);

	void DrawLineColorfade(const ax::Point& pt1, const ax::Point& pt2,
		const ax::Color& c1, const ax::Color& c2, float width = 1.0);
	...
	void DrawTexture(uint32_t texture, const ax::Rect& rect,
		ax::Color color = ax::Color(0.0f, 0.0f, 0.0f));

	void DrawBigImageResize(ax::BigImage* img, const ax::Point& position,
		const ax::Size& size, float alpha = 1.0);
