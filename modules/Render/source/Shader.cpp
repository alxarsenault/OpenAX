...
namespace GL {
	...
	Shader::Shader(const bool& content, const std::string& s_vextex_path,
	...
	void Shader::CompileAndLink()
	{
		...
		if (!_s_vertex_path.empty()) {
			...
			if (_is_content) {
				...
				std::vector<char> vertex_shader_buffer(
					(std::istreambuf_iterator<char>(vextex_shader_file)),
			...
			if (isCompiled == GL_FALSE) {
				...
				if (info_length > 1) {
					...
					glGetShaderInfoLog(
						s_vextex, info_length, &n_char_written, error_msg);

					ax::Error("Vertex Shader :", error_msg);
					delete[] error_msg;
				}
			}
		}
		...
		if (!_s_fragments_path.empty()) {
			...
			if (isCompiled == GL_FALSE) {
				...
				if (info_length > 1) {
					...
					char* error_msg = new char[info_length];
					glGetShaderInfoLog(
						s_fragments, info_length, &n_char_written, error_msg);

					ax::Error("Fragment Shader :", error_msg);
					delete[] error_msg;
				}
			}
		}
		...
		if (!linked) {
			...
			if (info_length > 1) {
				...
				ax::Error("Shader link :", error_msg);
				delete[] error_msg;
			}
