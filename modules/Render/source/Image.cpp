...
//	}
	...
	if (color_type == 2) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, _size.x, _size.y, 0, GL_RGB,
			GL_UNSIGNED_BYTE, image_data);
	}
	else {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _size.x, _size.y, 0, GL_RGBA,
			GL_UNSIGNED_BYTE, image_data);
	}
	...
	, _path("tmp")
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, c_type, _size.x, _size.y, 0, c_type,
...
bool Image::SaveImage(const std::string& path)
{
	...
	png_set_IHDR(png_ptr, info_ptr, _size.x, _size.y, depth, PNG_COLOR_TYPE_RGBA,
	...
	for (int y = 0; y < _size.y; ++y) {
		png_byte* row = (png_byte*)png_malloc(png_ptr, sizeof(uint8_t) * _size.x * pixel_size);
...
bool BigImage::InitImage(const std::string& path)
{
	...
	if (color_type == 2) {
		_imgData = new unsigned char[_size.x * _size.y * 3 * sizeof(unsigned char)];
		_colorType = RGB;
		memcpy(_imgData, image_data, size_t(_size.x * _size.y * 3 * sizeof(unsigned char)));
	}
	else {
		_imgData = new unsigned char[_size.x * _size.y * 4 * sizeof(unsigned char)];
		_colorType = RGBA;
		memcpy(_imgData, image_data, size_t(_size.x * _size.y * 4 * sizeof(unsigned char)));
	}
