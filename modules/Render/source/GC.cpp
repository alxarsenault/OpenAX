...
void ax::GC::DrawRectangle(const ax::Rect& rect)
{
	...
	// Order : bl, tl, tr, br.
	ax::Utils::RectPoints<ax::FloatPoint> points = frect.GetPoints();
	...
	const ax::Rect& rect, const int& radius)
{
	...
	glVertex2d(frect.position.x + frect.size.x - r + 2, frect.position.y);
	...
	glVertex2d(frect.position.x, frect.position.y + frect.size.y - r + 2);
	...
	const double& angle, const int& nSegments)
{
	...
	points.push_back(ax::FloatPoint(pos.x, pos.y));
	...
	for (int i = 0; i <= nSegments; i++) {
		...
		double x = radius * cosf(theta + angle);
		double y = radius * sinf(theta + angle);
		points.push_back(ax::FloatPoint(x + pos.x, y + pos.y));
		colors.push_back(c0);
	}
	...
	const ax::Size& sizeInImage, const ax::Point& position, float alpha)
{
	...
	float vertices[8] = {
		(float)pos.x, (float)pos.y, // Top-left.
		(float)pos.x, (float)pos.y + sizeInImage.y - 1, // Bottom-left.
		(float)pos.x + sizeInImage.x - 1,
		(float)pos.y + sizeInImage.y - 1, // Bottom-right.
	...
	const ax::Size& sizeInImage, const ax::Rect& rect, float alpha)
{
	...
	float vertices[8] = {
		(float)pos.x, (float)pos.y, // Top-left.
		(float)pos.x, (float)pos.y + rect.size.y, // Bottom-left.
		(float)pos.x + rect.size.x, (float)pos.y + rect.size.y, // Bottom-right.
	...
	ax::Font& font, const std::string& text, const ax::Rect& rect)
{
	if (font) {
		...
		ax::Point pos(5, rect.position.y + ceil((rect.size.y - height) * 0.5));
...
{
	ax::FloatPoint point(pt.Cast<float>());
	...
	const ax::Point& pos, const float& radius, const int& nSegments)
{
	...
	for (int i = 0; i < nSegments; i++) {
		...
		points[i] = ax::FloatPoint(x + real_pos.x, y + real_pos.y);
	}
	...
	const ax::Size& size, float alpha)
{
	...
	if (err != GL_NO_ERROR) {
		ax::Error("Can't draw axBigImage in GC : ", err);
	}
